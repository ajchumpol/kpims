	...
	<tr>
		<td style="vertical-align:top;border:0px;">
		  ...
		  <div class="w3-container">
			...
			echo form_open('ManageDocument/updating', $attributes);
			?>
		  <table class="w3-table w3-light-grey">
			<tr>
				...
				<td colspan="3"><input class="w3-input w3-border" style="size:300px;" type="text" name="i_title" value="<?=$doc_title?>" required></td>
			</tr>
			<tr>
				<td>สถานะเอกสาร</td>
			...
			<tr>
				<td></td><td></td>
				<td>ผู้แก้ไข</td>
				<td><input class="w3-input w3-border w3-light-grey" type="text" name="i_edit_by" value="<?=$_SESSION['s_user_flname']?>" readonly required></td>
				<td>วันที่แก้ไข</td>
				<td><input class="w3-input w3-border w3-light-grey" type="text" name="i_edit" value="<?=date('Y-m-j')?>" readonly required></td>
			</tr>
		  </table>
		  ...
		  <div class="w3-container">
		  	<table class="w3-table">
			    <tr class="w3-blue">
			      ...
			      <th class="w3-center" colspan="3">แนวทางการกำหนดตัวชี้วัด</th>
			    </tr>
		  <?php
		  	$ind = 0;
		  ...
		  ?>
			<tr class="w3-grey">
				...
				<td colspan="3"><?=$data_cri_arr[$j_no]['cri_kpi_appexa']?></td>
			</tr>
			...
			<tr>
				...
				<td class="w3-center"><input type="number" min="-<?=$data_cri_arr[$j_no]['cri_wei_min']?>" max="<?=$data_cri_arr[$j_no]['cri_wei_max']?>" name="i_cokpi_wei[<?=$data_cri_arr[$j_no]['cri_id']?>][<?=$data_cri_arr[$j_no]['cokpi_id']?>]" value="<?=$data_cri_arr[$j_no]['cokpi_score']?>" style="width:50px;" required><span style="color:#FF0000;">*</span></td>
				<td colspan="4"><?=$data_cri_arr[$j_no]['cokpi_app']?></td>
			</tr>
		  <?php
		  ...
		  ?>
			<tr class="w3-light-grey">
				...
				<td colspan="4"><?=$data_cokpi_arr[$i_no]['subcokpi_def']?><input type="hidden" name="i_cokpi_subkpi[<?=$data_cokpi_arr[$i_no]['cokpi_id']?>][<?=$data_cokpi_arr[$i_no]['subcokpi_id']?>]" value="<?=$data_cokpi_arr[$i_no]['subcokpi_id']?>"></td>
			</tr>
		  <?php
		  ...
		  ?>
				<table class="w3-table w3-light-grey">
					...
					<tr>
						<td><input type="radio" name="i_gra_score[<?=$data_cokpi_arr[$i_no]['subcokpi_id']?>][<?=$data_subcokpi_arr[$k_no]['issdet_id']?>]" value="<?=$data_grade_arr[$l_no]['gra_score1']?>" <?=$chk1?> required><?=check_value($data_grade_arr[$l_no]['gra_title1'])?></td>
						<td><input type="radio" name="i_gra_score[<?=$data_cokpi_arr[$i_no]['subcokpi_id']?>][<?=$data_subcokpi_arr[$k_no]['issdet_id']?>]" value="<?=$data_grade_arr[$l_no]['gra_score2']?>" <?=$chk2?> required><?=check_value($data_grade_arr[$l_no]['gra_title2'])?></td>
						<td><input type="radio" name="i_gra_score[<?=$data_cokpi_arr[$i_no]['subcokpi_id']?>][<?=$data_subcokpi_arr[$k_no]['issdet_id']?>]" value="<?=$data_grade_arr[$l_no]['gra_score3']?>" <?=$chk3?> required><?=check_value($data_grade_arr[$l_no]['gra_title3'])?></td>
						<td><input type="radio" name="i_gra_score[<?=$data_cokpi_arr[$i_no]['subcokpi_id']?>][<?=$data_subcokpi_arr[$k_no]['issdet_id']?>]" value="<?=$data_grade_arr[$l_no]['gra_score4']?>" <?=$chk4?> required><?=check_value($data_grade_arr[$l_no]['gra_title4'])?></td>
						<td><input type="radio" name="i_gra_score[<?=$data_cokpi_arr[$i_no]['subcokpi_id']?>][<?=$data_subcokpi_arr[$k_no]['issdet_id']?>]" value="<?=$data_grade_arr[$l_no]['gra_score5']?>" <?=$chk5?> required><?=check_value($data_grade_arr[$l_no]['gra_title5'])?></td>
					</tr>
				</table>
			<?php
								$ind++;
				...
				<td>
					...
					for ($l_no = 0; $l_no < count($data_att_arr); $l_no++):
						if (($data_att_arr[$l_no]['issdet_id'] == $data_subcokpi_arr[$k_no]['issdet_id']) && 
							($data_att_arr[$l_no]['subcokpi_id'] == $data_subcokpi_arr[$k_no]['subcokpi_id'])):
							$flag++;
